...
class baseModel {
    ...
    async findAll(filter) {
        ...
        //     // search
        //     if (item.field === 'productName' && item.operator === 'includes') {
        //         queryDb[item.field] = { $regex: item.value, $options: 'i' };
        //     }

        //     if (item.field === 'categoryCode' && item.operator === 'includes' && item.value.length) {
        //         queryDb[item.field] = { $in: item.value };
        //     }

        //     // find less than
        //     if (item.operator === 'less' && item.value) {
        //         queryDb[item.field] = { $lt: item.value };
        //     }
        ...
        //     }
        // });
        const dataArray = this.collection.find(queryDb).toArray();
        // find data
